...
async def cmd_start(message: types.Message):
    ...
    await message.answer(
        f"🌟 <b>Gacha Battle</b> 🌟\n\n"
        f"Собирай уникальных персонажей, открывай ящики и становись лучшим!\n\n"
...
async def cmd_daily(message: types.Message):
    ...
    if success:
        await message.answer(
            f"🎉 Ежедневный бонус получен! +10 {CURRENCY_NAME}\n"
...
async def cmd_top(message: types.Message):
    ...
    for i, entry in enumerate(lb, 1):
        ...
        text += f"{medal} <b>{name}</b> — {entry['unique_cards']} карт, {entry['total_legendaries']} лег.\n"
    await message.answer(text, parse_mode=ParseMode.HTML, reply_markup=main_keyboard())
...
async def cmd_equip(message: types.Message):
    args = message.text.split()
    if len(args) < 3:
        await message.answer("❌ Использование: /equip <id_карты> <слот>\nСлоты: weapon, armor, accessory, mount\n\nНапример: /equip 5 weapon")
        return
    try:
        card_id = int(args[1])
        slot = args[2].lower()
        success, msg = await db.equip_card(message.from_user.id, card_id, slot)
        await message.answer(msg if success else f"❌ {msg}", reply_markup=main_keyboard())
    except ValueError:
...
async def cmd_unequip(message: types.Message):
    args = message.text.split()
    if len(args) < 2:
        await message.answer("❌ Использование: /unequip <слот>\nСлоты: weapon, armor, accessory, mount")
        return
    slot = args[1].lower()
...
async def cmd_kit(message: types.Message):
    ...
    for slot, card in equipped.items():
        if card:
            has_any = True
            text += f"{slot.upper()}: {card['emoji']} {card['name']} (⚡{card.get('attack',0)}🛡{card.get('defense',0)}❤️{card.get('hp',0)})\n"
        else:
            text += f"{slot.upper()}: пусто\n"
    if not has_any:
        text += "Ничего не экипировано. Используй /equip!"
    await message.answer(text, parse_mode=ParseMode.HTML, reply_markup=main_keyboard())
...
async def cmd_pvp(message: types.Message):
    args = message.text.split()
    if len(args) < 2:
        await message.answer("❌ Использование: /pvp <telegram_id>\n\nНапример: /pvp 123456789")
        return
    try:
        ...
        text = (
            ...
            f"👤 Вы: {c['team'][0]['emoji']} {c['team'][0]['name']} (❤️{c['hp_left']}/{c['total_hp']})\n"
            f"👤 {t['username']}: {t['team'][0]['emoji']} {t['team'][0]['name']} (❤️{t['hp_left']}/{t['total_hp']})\n"
...
async def cb_shop(callback: types.CallbackQuery):
    text = "⭐ <b>Магазин Stars</b>\n\nКупи монеты за Telegram Stars:\n\n"
    for i, pkg in enumerate(STARS_PACKAGES):
        text += f"{i + 1}. {pkg['stars']} ⭐ → {pkg['coins']} {CURRENCY_NAME} + {pkg['boxes']} ящиков\n"
    text += "\nСкоро будет доступно через Bot API!"
        ...
        for user in users:
            try:
                await bot.send_message(
                    user["telegram_id"],
                    f"🎁 Твой ежедневный ящик готов!\n"
                    f"Напиши /daily чтобы забрать, или открой Mini App!",
                    reply_markup=main_keyboard()
                )
            except Exception:
                pass
